  ...
  class MatchCommitter
    ...
    def commit_match(candidate, a, b)
      match = Match.create!(
        ...
        reasoning: reasoning_for(candidate, a, b)
      )
      MatchLeg.create!(
        match: match,
        reconcilable_item: a,
    ...
    def reasoning_for(candidate, a, b)
      ...
      fragments = Array(candidate.matched_on[:reasoning_fragments]).compact
      return reasoning if fragments.empty?

      "#{reasoning}, #{fragments.join(', ')}"
    end
  end
end
